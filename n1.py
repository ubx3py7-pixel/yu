...
This file includes an owner-only /botstat command that shows overall bot stats and top users by credits.
"""
...
TEMPLATE_NOT_FOUND = """🔍 No results found.
Zoro lost the battle 😔.

Remaining credits: {credits_left}
"""

# Short user-facing messages
WELCOME_TEXT = "👋 Welcome to FIRE DROP OSINT bot.\nType /help to see the command menu."
HELP_TEXT = (
    "🛠️ <b>FIRE DROP - Commands</b>\n\n"
    "🔎 <b>/num</b> - Check mobile number info\n"
    "🆔 <b>/addhar</b> - Aadhar lookup (API not added yet)\n"
    "🚗 <b>/gaddi</b> - Vehicle lookup (API not added yet)\n"
    "💎 <b>/credits</b> - Show your credits\n\n"
    "Usage examples:\n"
    "/num 9259013600\n"
    "/addhar 123456789012\n"
    "/gaddi UP32AB1234\n\n"
    "Note: This bot uses OSINT/leaked databases which may be inaccurate. Use responsibly."
)

SLOWMODE_MSG = ("⏳ <b>Slow mode active</b> ❗\n"
                "Try again in {seconds} seconds.\n\n"
                "To remove slow mode DM @Firedrop_69 — we can help for a small fee.")

NO_CREDITS_MSG = "💔 You have no credits left. DM @Firedrop_69 to buy more credits."
...
# New helpers for /botstat
def get_total_credits():
    conn = get_conn()
    cur = conn.cursor()
    # COALESCE used to be safe if SUM returns NULL
...
def get_top_users(limit: int = 20):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT tg_id, username, credits FROM users ORDER BY credits DESC LIMIT ?", (limit,))
    rows = cur.fetchall()
    conn.close()
    return rows
...
async def process_message(session, msg):
    ...
    async def pre_check():
        ...
        if slowed_until and slowed_until > now:
            ...
            return SLOWMODE_MSG.format(seconds=seconds_left)
        return None

    # /start
    if cmd == "/start":
        await send_message(session, chat_id, WELCOME_TEXT)
        return
    ...
    # Quick responses for APIs not added yet: Aadhar & Gaddi
    if cmd in ("/addhar", "/aadhar", "/aadhaar"):
        await send_message(session, chat_id, "🔧 Aadhar API is not added yet — we're working on it. Please try again later.")
        return

    if cmd == "/gaddi":
        await send_message(session, chat_id, "🔧 Vehicle (gaddi) API is not added yet — we're working on it. Please try again later.")
        return

    # lookup commands (currently only /num will call the OSINT API)
    if cmd in ("/num",):
        if not arg:
            usage = {
        ...
        credits = user[2] if user else 0
        if credits <= 0:
            await send_message(session, chat_id, NO_CREDITS_MSG)
            return
        ...
        if cmd_c >= SLOWMODE_AFTER:
            ...
            await send_message(session, chat_id, ("⏳ Slow mode is active ❗\n"
                                                  "To remove slow mode DM @Firedrop_69."))
        return
# /redeem
    if cmd == "/redeem":
        if not arg:
    ...
    # /botstat (owner-only): show total users, total credits, avg, top users
    if cmd in ("/botstat", "/botstats", "/stats"):
        if not is_owner(user_id):
            await send_message(session, chat_id, "Unauthorized.")
            return

        # allow optional arg to set top-N, e.g. /botstat 50
        top_n = 20
        try:
            if arg:
                top_n = max(1, min(200, int(arg.strip())))
        except:
            top_n = 20

        total_users = get_user_count()
        total_credits = get_total_credits()
        avg_credits = (total_credits / total_users) if total_users else 0.0
        top = get_top_users(top_n)
        ...
        lines = [
            ...
            f"📈 Average credits/user: {avg_credits:.2f}",
            "",
            f"<b>Top users (by credits) — top {len(top)}</b>:",
        ]

        if top:
            for uid, uname, creds in top:
                uname_disp = uname if uname else "—"
                lines.append(f"{uid} | {uname_disp} | {creds} credits")
        else:
            lines.append("No users yet.")

        msg = "\n".join(lines)
        await send_message(session, chat_id, msg, parse_mode="HTML")
        return
    ...
    if cmd == "/rem":
        ...
        try:
            await send_message(session, uid, "Hello, Thanks for supporting us — here is 5 free credits and slow mode removed ✅")
        except Exception:
            pass
